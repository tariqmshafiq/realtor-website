...
Everything is personalized per request — the page `<title>`, the hero, the "about" section, the review headlines, the contact block, and the `tel:` / `mailto:` links. All parameters are optional; the page renders sensible defaults without them.

## Outreach workflow (send to many realtors)

1. Deploy once (already live at `https://dmvwebdev.vercel.app/`).
2. In a spreadsheet, add columns: `name`, `co`, `email`, `phone`, `line`.
